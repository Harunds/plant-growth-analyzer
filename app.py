...
class StreamlitPlantAnalyzer:
    ...
    @st.cache_data
    def load_and_process_data(_self, data_source):
        """Load and process data from a given source (file path or URL)"""
        try:
            # Check if data_source is a string (URL) or an uploaded file object
            if isinstance(data_source, str):
                df = pd.read_csv(data_source)
            else: # Assume it's a file-like object from st.file_uploader
                df = pd.read_csv(data_source)
...
def main():
    ...
    # Definisi URL file CSV dari GitHub
    # Pastikan ini adalah 'raw' URL dari file di GitHub
    github_csv_url = "https://raw.githubusercontent.com/Harunds/plant-growth-analyzer/main/data%20combined%20new.csv"
    ...
    if df is not None:
        ...
        with st.expander("📋 Format CSV yang Diharapkan"):
            ...
            st.write("- `minggu`: Nomor minggu (integer)")
            st.write("- `tinggi_cm`: Tinggi tanaman dalam sentimeter (desimal)")
            st.write("- `diameter_cm`: Diameter batang dalam sentimeter (desimal)")
            st.write("- `jumlah_kanopi`: Jumlah kanopi/daun (integer)")
            st.write("- `plot`: Pengidentifikasi plot (teks/angka)")
